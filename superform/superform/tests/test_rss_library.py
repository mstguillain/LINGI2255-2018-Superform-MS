...
def test_date():
    assert get_element('pubDate', 'Mon, 19 Nov 2018 08:00:00 GMT') in Feed('', '', '',
                                                                           pubDate=datetime.datetime(2018, 11, 19, 8, 0,
                                                                                                     0)).rss()
...
def test_optionaltest_elements_feed():
    ...
    assert get_element('pubDate', 'Thu, 13 Nov 2014 08:00:00 GMT') in Feed('', '', '',
                                                                           pubDate=datetime.datetime(2014, 11, 13, 8, 0,
                                                                                                     0)).rss()
    assert get_element('lastBuildDate', 'Mon, 01 Dec 2014 10:22:15 GMT') in Feed('', '', '',
                                                                                 lastBuildDate=datetime.datetime(2014,
                                                                                                                 12, 1,
                                                                                                                 10, 22,
                                                                                                                 15)).rss()
...
def test_skipdaystest_element_feed():
    rss = Feed('', '', '',
               skipDays=SkipDays(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Friday'])).rss()
...
def test_optionaltest_elements_item():
    ...
    assert get_element('link', 'http://example.com/') in Feed('', '', '',
                                                              items=[Item(title='', link='http://example.com/')]).rss()
    ...
    assert get_element('author', 'email@example.com') in Feed('', '', '',
                                                              items=[Item(title='', author='email@example.com')]).rss()
    assert get_element('dc:creator', 'Sample Example') in Feed('', '', '',
                                                               items=[Item(title='', creator='Sample Example')]).rss()
    assert get_element('comments', 'Sample comment') in Feed('', '', '',
                                                             items=[Item(title='', comments='Sample comment')]).rss()
    assert get_element('pubDate', 'Thu, 13 Nov 2014 08:00:00 GMT') in Feed('', '', '', items=[
