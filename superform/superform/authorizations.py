    ...
    if request.method == "GET":
        ...
        while i <= (round(len(request.form) / 3)):
            ...
            if user_id is not "":
                ...
                a = Authorization(channel_id = channel_id, user_id = user_id,
                                  permission = permission)
                print(a)
                db.session.add(a)
            i = i + 1

        edit_list = [[elem, elem.split('#')] for elem in request.form if
                     elem.startswith("permission_edit")]

        for e in edit_list:
            auth = request.form.get(e[0])
            chan_id = e[1][2]
            user = e[1][1]
            a = Authorization.query.filter(Authorization.user_id == user,
...
def delete_authorization(id, cid):
    auth = Authorization.query.filter(Authorization.user_id == id,
                                      Authorization.channel_id == cid).first()
